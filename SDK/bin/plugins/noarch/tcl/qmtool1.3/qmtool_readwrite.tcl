...
# be parsed.
#
# $Id: qmtool_readwrite.tcl,v 1.46 2012/01/27 20:52:26 johns Exp $
#
...
# 	 } elseif { $flag=="B" || ![llength $flag]} { incr nmods; continue }
	 ...
	 incr nbonds
	 set name R$nbonds
	 # Check for "B * * R" --> remove all bonds
	 ...
	 incr nangles
	 set name A$nangles
	 # Check for "A * * * R" --> remove all angles
	 if {$wild==$nind && [string match "*R*" $flag]} { 
	 ...
	 if {$gtype=="O"} {
	    ...
	    incr nimprops
	    set name O$nimprops
	    # Check for "O * * * * R" --> remove all impropers
	    if {$wild==$nind && [string match "*R*" $flag]} { 
	    ...
	    incr ndiheds
	    set name D$ndiheds
	    # Check for "D * * * * R" --> remove all dihedrals
	    if {$wild==$nind && [string match "*R*" $flag]} { 
...
proc ::QMtool::read_gaussian_input { file {intomolid -1} } {
   ...
   if {$havecart} {
      ...
      if {[file exists $tmppdb]} { catch {file delete $tmppdb} }
...
proc ::QMtool::read_gaussian_log { file {intomolid -1}} {
   ...
   if {$zmatgiven} {
	 ...
	 # definition of internal coordinates, then incorrect stuff would be returned.
 ...
 # endif  
   ...
   if {$simtype=="Geometry optimization" || [string match {CBS-*} $method] || \
	 ...
	 if {$intomolid>=0 && [lsearch [molinfo list] $intomolid]>=0} {
	    mol addfile $tmppdb molid $intomolid waitfor all
	    #puts "imol=$intomolid"
	 } else {
	    set newmolid [mol new $tmppdb type pdb waitfor all]
	    #puts "mol=$newmolid"
	 }
...
proc ::QMtool::reload_cartesians_as_pdb { {pdbfile {}} {intomolid -1} } {
   ...
   # If a molid was specified in the argument list load coordinates into that molecule
   if {$intomolid>=0 && [lsearch [molinfo list] $intomolid]>=0} {
      set newmolid [mol addfile $pdbfile type pdb molid $intomolid waitfor all]
   } else {
      set newmolid [mol new $pdbfile type pdb waitfor all]
      molecule_add $newmolid
   }
