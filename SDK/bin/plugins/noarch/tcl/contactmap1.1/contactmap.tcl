...
# vmd@ks.uiuc.edu
#
# $Id: contactmap.tcl,v 1.49 2009/06/30 23:27:41 akohlmey Exp $
#
#
# To try out:
# 0) start vmd
      ...
      for {set field  $firstData}  {$field <= $fieldLast} {incr field} {
        ...
        for {set yPos 0} {$yPos < $dataValANum} {incr yPos} { 
          ...
          if {$val != "null"} {
            ...
            if {$dataName($field) != "struct"} {
              set range [expr $dataMax - $dataMin]
              if { ($range > 0)  && ([string is double $val] )} {
                set intensity  [expr {int (255 * ( ($val - $dataMin ) / $range))}]
              }
...
proc ::contactmap::contactmapMain {} {
  ...
  if {$usableMolLoaded} {
    ...
    wm title $w "VMD Contact Map: $currentMolA_name (mol $currentMolA)   $currentMolB_name (mol $currentMolB)"
    ...
    set fit_scaley 1.0
    if {$dataValANum >= 0} {
        set fit_scaley [expr {(0.0 + $ycanwindowmax - $ytopmargin - $ybottommargin) / ($ybox * ($dataValANum + 1) )}];    #since we zero-count.
    }

    set scaley 1.0
    set scalex $fit_scalex 
    verbose "Restarting data, scalex= $scalex, scaley= $scaley, fit_scalex= $fit_scalex, fit_scaley= $fit_scaley"
...
proc ::contactmap::molChooseMenu {name function op} {
  ...
  if {$molList == ""} {
    ...
    # Handle the deletion of the selected molecule by setting both currentMol's to first in list
    # do MolB first, before trace moves execution.
    #XXX improve so molChoose triggered by either A or B, with less fragile dependence on trace
   if {[lsearch -exact $molList $currentMolB] == -1} {
      set currentMolB [lindex $molList 0]
    }
    if {[lsearch -exact $molList $currentMolA] == -1} {
      set cma [lindex $molList 0]
      #verbose "cma is $cma"
      if {![info exists rep($cma)]} {
        set rep($cma) "null"
      }
      #now set to be caught by trace
      set currentMolA $cma
    }
...
proc ::contactmap::getStartedMarquee {x y shiftState whichCanvas} {
  ...
  if {$usableMolLoaded} {
    ...
    switch -exact $startCanvas {
      ...
      default {verbose "problem with finding canvas..., startCanvas= >$startCanvas<"} 
    }   
...
proc ::contactmap::molChoose {name function op} {
  ...
  if {![info exists rep($currentMolA)]} {
    set rep($currentMolA) "null"
  }
  ...
  # to STRIDE) if contactmap window is showing
...
proc ::contactmap::keepMovingMarquee {x y whichCanvas} {
  ...
  if {$usableMolLoaded} {
    ...
    switch -exact $startCanvas {
      ...
      vert {set drawCan $w.cfr.vertScale}
      horz {set drawCan $w.cfr.horzScale}
      default {verbose "problem with finding canvas (moving marquee)..., startCanvas= $startCanvas"}
    } 
...
proc ::contactmap::initVars {} {
  #variable verboseChannel stdout 
  variable verboseChannel {} 
  ...
  variable xcanwindowStarting 480
  variable ycanwindowStarting 460 
...
proc  ::contactmap::showPair {x y shiftState whichCanvas} {
  ...
  if {($dataValANum <= 0) || ($dataValBNum <= 0)} {
    return
  }
...
proc ::contactmap::calcDataDist {} {
  ...
  for {set indexA 0} {$indexA <= $dataValANum} {incr indexA} {
    ...
    #XXX effective range is hard-set here. ignores actual vaiues
    set dataMin 0 
    set dataMax 10 
  }
  unset coorListA
