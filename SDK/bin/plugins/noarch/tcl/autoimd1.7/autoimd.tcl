...
# AutoIMD
#
# $Id: autoimd.tcl,v 1.98 2009/12/06 23:08:08 ltrabuco Exp $
#
...
namespace eval AutoIMD {
  ...
  namespace eval settings {
    ...
    global env
    variable namdtmplfile   
    if {[info exists env(AUTOIMDDIR)]} {
      set namdtmplfile [file join $env(AUTOIMDDIR) autoimd-template.namd]
    }
    ...
    variable usecispeptide 0
    # Interface with the chirality plugin
    variable usechirality 0
...
proc AutoIMD::savestructure_core { } {
  ...
  # If called from the cispeptide plugin, create an extrabonds file
  # containing dihedral restraints to fix cis peptide bonds.
  if $settings::usecispeptide {
    package require cispeptide
    cispeptide extrab [file join "$settings::scratchdir" autoimd-cispeptide-extrab.txt] [file join "$settings::scratchdir" autoimd.pdb]
  }
  # If called from the chirality plugin, create an extrabonds file
  # containing improper restraints to fix chirality errors.
  if $settings::usechirality {
    package require chirality
    chirality extrab [file join "$settings::scratchdir" autoimd-chirality-extrab.txt] [file join "$settings::scratchdir" autoimd.pdb]
  }
...
proc AutoIMD::generate_namd_conf { } {
  ...
  puts $conffile "set usecispeptide $settings::usecispeptide"
  puts $conffile "set usechirality $settings::usechirality"
...
if {0} { ;# Old code
  ...
  # Set up appropriate reps if called from the cispeptide plugin
  if $::AutoIMD::settings::usecispeptide {
    ::cispeptide::cispeptide_autoimd_reps
  }
  # Set up appropriate reps if called from the chirality plugin
  if $::AutoIMD::settings::usechirality {
    ::chirality::chirality_autoimd_reps
  }
...
proc AutoIMD::finish { args } {
  ...
  # Turn original molecule back on if called from the cispeptide plugin
  if $::AutoIMD::settings::usecispeptide {
    ::cispeptide::cispeptide_autoimd_finish
  }
  # Turn original molecule back on if called from the chirality plugin
  if $::AutoIMD::settings::usechirality {
    ::chirality::chirality_autoimd_finish
  }
