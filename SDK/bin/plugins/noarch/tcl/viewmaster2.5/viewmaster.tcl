# ViewMaster version 2.4
# Copyright (C) 2004 Justin Gullingsrud <jgulling@mccammon.ucsd.edu>
#
# $Id: viewmaster.tcl,v 1.12 2012/01/26 21:27:32 johns Exp $
#
...
package provide ViewMaster 2.5
...
proc ViewMaster::get_next_id {} {
   variable current_view
   variable imageid

   set retVal [expr {$current_view + 1} ]
   if { $retVal == $imageid } {
      return 0
   } else {
      return $retVal
...
proc ViewMaster::get_prev_id {} {
   variable current_view
   variable imageid

   if { $current_view == 0} {
      return [expr {$imageid - 1}]
   } else {
      return [expr {$current_view - 1}]
   }
}

proc ViewMaster::set_Aux_keys {} {
   user add key Aux-0 { ViewMaster::do_restore_prev }
   user add key Aux-1 { ViewMaster::do_restore_next }
   user add key Aux-2 { ViewMaster::do_save }
}

proc ViewMaster::do_restore_next {  } {
   do_restore [ ViewMaster::get_next_id ]
}

proc ViewMaster::do_restore_prev {  } {
   do_restore [ ViewMaster::get_prev_id ]
}
...
proc ViewMaster::load_materials { matprops } {
  ...
  foreach { mat prop } $matprops {
    ...
    foreach { ambn spec diff shin opct outln outlnw transmode } $prop {
      ...
      material change outline       $mat $outln
      material change outlinewidth  $mat $outlnw
      material change transmode     $mat $transmode
    }
  }
}
