...
##          vmd@ks.uiuc.edu
##
## $Id: biocorelogin.tcl,v 1.25 2008/04/15 16:49:47 kvandivo Exp $
##
## Home Page:
##   http://www.ks.uiuc.edu/Research/vmd/plugins/biocore
##
package provide biocorelogin 1.01
...
proc ::BiocoreLogin::doLogin {userName passWord} {
   variable header "Error Message Trying To Login:\n\n"
   if {[string equal "" [string trim $userName]]} {
      tk_messageBox -type ok -message "$header Username must be provided" -title "Error!"
   } elseif {[string equal "" [string trim $passWord]]} {
      tk_messageBox -type ok -message "$header Password must be provided" -title "Error!"
   } elseif {[string equal "" [string trim $::biocore::URL ]]} {
      tk_messageBox -type ok -message "$header Server URL must be provided" -title "Error!"
   } else {
#     set ::biocore::URL $::BiocoreLogin::m_server 
      if {[catch {eval ::biocore::ui configure [::biocore::login \
         ...
         tk_messageBox -type ok -message "$header $msg" -title "Error!"
...
#puts 6
    ...
    the Biological Collaborative Research Environment, from within VMD you must first log in. \
