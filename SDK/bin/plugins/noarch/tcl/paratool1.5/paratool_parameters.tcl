...
proc ::Paratool::choose_analog_parameters_gui {} {
   ...
   button $w.buttons.add -text "Use parameters"    -command {
	 ...
	 lset ::Paratool::zmat $izmat 4 [lrange [lindex $::Paratool::chooseanaloglist $i] end-3 end-1]; # Kchi, n, delta
...
proc ::Paratool::get_analog_bond_params { elemtypelist {knowntypes {}}} {
   variable paramsetlist
   variable chooseanalogformat [format "%14s  %8s %8s  %s" Atomtypes K_r r_0 Comment]
   set bondparamlist {}
   foreach paramset $paramsetlist {
   ...
   foreach pos $poslist {
      ...
      lappend bondcandidatelist [format "%6s %6s  %8.4f %8.4f  %s" [lindex $bond 0 0] [lindex $bond 0 1] \
...
proc ::Paratool::get_analog_angle_params { elemtypelist {knowntypes {}} } {
   variable paramsetlist
   variable chooseanalogformat [format "%20s  %8s %8s %8s %8s  %s" Atomtypes K_theta Theta_0 K_ub S_0 Comment]
   set angleparamlist {}
   foreach paramset $paramsetlist {
   ...
   foreach pos $poslist {
      ...
      lappend anglecandidatelist [format "%6s %6s %6s  %8.2f %8.4f %8s %8s %s" [lindex $angle 0 0] [lindex $angle 0 1] \
...
proc ::Paratool::get_analog_dihed_params { elemtypelist {knowntypes {}} } {
   ...
   variable paramsetlist
   variable chooseanalogformat [format "%27s  %8s %2s %8s  %s" Atomtypes K_chi n delta Comment]
   set dihedparamlist {}
   foreach paramset $paramsetlist {
   ...
   foreach pos $poslist {
      ...
      lappend dihedcandidatelist [format "%6s %6s %6s %6s  %8.4f %2i %8.1f  %s" [lindex $dihed 0 0] [lindex $dihed 0 1] \
...
proc ::Paratool::get_analog_imprp_params { elemtypelist {knowntypes {}} } {
   ...
   variable chooseanalogformat [format "%27s  %8s %8s  %s" Atomtypes K_psi psi_0 Comment]
   variable paramsetlist
   set imprpparamlist {}
   foreach paramset $paramsetlist {
   ...
   foreach pos $poslist {
      ...
      lappend imprpcandidatelist [format "%6s %6s %6s %6s  %8.4f %8.1f  %s" [lindex $imprp 0 0] [lindex $imprp 0 1] \
