...
#
# $Id: paratool_readwrite.tcl,v 1.42 2011/06/17 00:34:45 saam Exp $
#
...
#   if {[llength $workdir]} { cd workdir }
   ...
   foreach file $topologyfiles {
      set file [string map [list \[DEFAULTDIR\] $env(CHARMMTOPDIR)] $file]
   ...
   foreach file $paramsetfiles {
      set file [string map [list \[DEFAULTDIR\] $env(CHARMMPARDIR)] $file]
...
proc ::Paratool::save_project { file } {
   ...
   set fid [open $file w]
   set paratoolversion [lindex [package versions paratool] end]
   puts $fid "variable savedstateparatoolvmdversion [vmdinfo version]"
   puts $fid "variable savedstateparatoolversion $paratoolversion"
