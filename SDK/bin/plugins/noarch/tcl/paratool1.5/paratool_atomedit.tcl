...
# Atom editing procs
#
# $Id: paratool_atomedit.tcl,v 1.45 2011/06/17 00:32:57 saam Exp $
#
...
proc ::Paratool::atomedit_update_list { {complexcheck check} } {
   ...
   foreach tag $atomproptags {
      switch $tag {
         ...
         Name    {
            append formatstring {%6s }
            append formatfield  {%6s }
         }
         Flags    {
            append formatstring {%5s }
            append formatfield  {%5s }
...
proc ::Paratool::atomedit_choose_type {} {
   ...
   foreach type $elemtypelist {
      ...
      lappend choosetypelist [format "%6s %10s %10s %10s %10s  %s" [lindex $type 0] $vdweps $vdwrmin \
   ...
   foreach uffentry $uff {
      ...
      lappend choosetypelist [format "%6s %10s %10s  %s" ${elem}U$i \
...
proc ::Paratool::atomedit_choose_type_gui {} {
   ...
   label $w.types.format -font $fixedfont -text [format "%6s %10s %10s %10s %10s  %s" Type VDWeps VDWrmin VDWeps14 VDWrmin14 Comment] \
...
proc ::Paratool::set_pickmode_atomedit {} {
   ...
   mouse mode pick
   #mouse callback on
   trace add variable ::vmd_pick_event write ::Paratool::atom_picked_fctn
   #puts "[trace info variable ::vmd_pick_event]"
