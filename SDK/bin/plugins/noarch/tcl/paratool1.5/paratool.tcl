...
# Paratool plugin
#
# $Id: paratool.tcl,v 1.109 2012/01/24 17:36:26 johns Exp $
#
...
namespace eval ::Paratool:: {
   ...
   proc initialize { } {
      ...
      variable buttonbgcolor SlateGray2
...
################################################
# Setup the default files and directories.     #
################################################

proc ::Paratool::setup_default_toppar_files {} {
   variable useamberparams
   global env
   if {!$useamberparams} {
      variable topologyfiles [file join {[DEFAULTDIR]} top_all27_prot_lipid_na.inp]
      variable paramsetfiles [file join {[DEFAULTDIR]} par_all27_prot_lipid_na.inp]
   } else {
      variable topologyfiles [file join {[DEFAULTDIR]} top_amber2charmm.inp]
      variable paramsetfiles [file join {[DEFAULTDIR]} par_amber2charmm.inp]
   }
   set expandedfile [string map \
          [list \[DEFAULTDIR\] $env(CHARMMTOPDIR)] $topologyfiles]
   variable topologylist [list [::Toporead::read_charmm_topology \
                         $expandedfile]]
   set expandedfile [string map \
          [list \[DEFAULTDIR\] $env(CHARMMPARDIR)] $paramsetfiles]

   variable paramsetlist [list [::Pararead::read_charmm_parameters \
                         $expandedfile]]
}
...
#   tk_setPalette background [.paratool cget -bg] \
#      selectbackground $selectcolor \
#      selectColor $buttonbgcolor
   ...
   $w.menu.file add command -label "Import Gaussian optimized geometry" \
...
proc ::Paratool::atom_picked_fctn { args } {
   ...
   #puts "pickmode=$pickmode; picked atom $vmd_pick_event; $picklist"
...
proc ::Paratool::opendialog { type {initialfile ""} } {
   ...
   if {$type=="topo"} {
      set types {
	 {{CHARMM topology files}       {.top .rtf top*.inp}  }
	 {{All files}        *            }
      }
   } elseif {$type=="para"} {
      set types {
	 {{CHARMM parameter files}       {.par .prm par*.rtf par*.inp}  }
	 {{All files}        *            }
      }
   } elseif {$type=="potscan"} {
      set types {
...
proc ::Paratool::toggle_charmm_amber {} {
   # Delete all existing topology files from list
   .paratool.topo.list.list selection set 0 end
   .paratool.topo.list.buttons.delete invoke

   # Delete all existing parameter files from list
...
proc ::Paratool::remove_traces {} {
   ...
   foreach t [trace info variable ::vmd_pick_event] {
      trace remove variable ::vmd_pick_event write ::Paratool::atom_picked_fctn
   }
