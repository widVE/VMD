...
#
# $Id: solvate.tcl,v 1.56 2012/01/12 21:06:29 johns Exp $
#
...
# Set some defaults
  ...
  if { $jsoutput && [molinfo top get numatoms] > 4000000 } { 
    ...
    $sel delete
    if { $num != 0 } {
      vmdcon -error "Found $num water atoms near the solute!  Please report this bug to"
      vmdcon -error "vmd@ks.uiuc.edu, including, if possible, your psf and pdb file."
      error "Solvate 1.3 failed."  
    }
  }

  vmdcon -info "Solvate 1.3 completed successfully."
  puts $logfile "Solvate 1.3 completed successfully."
...
proc ::Solvate::run_solvate {} {
  ...
  if { !$waterbox } {
    if { ($psffile == {}) || ($pdbfile == {} ) } {
      vmdcon -error "solvate: need file names"
      return
    }
  ...
  if { $outprefix == {} } {
    vmdcon -error "solvate: need output filename"
    return
  }
  set command_line [concat $command_line "-o" $outprefix]

  if { $segid == {} } {
    vmdcon -error "solvate: need segid"
    return
  }
  ...
  if { !$use_mol_box } {
    if { ![eval ::Solvate::is_number $min(x)] ||
      ...
      vmdcon -error "solvate: need numeric minmax"
      return
    }
  ...
  if { ![eval ::Solvate::is_number $minpad(x)] ||
    ...
    vmdcon -error "solvate: need numeric padding"
    return
  }
