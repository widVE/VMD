...
#
# (c) 2004-2010 by Axel Kohlmeyer <akohlmey@gmail.com>
#     
# the clone all/active/displayed molecule feature based on 
# a patch provided by Luis Gracia <lug2002@med.cornell.edu>
########################################################################
#
...
package provide clonerep $::CloneRep::version
...
proc ::CloneRep::clone_reps {{fromid top} {toid all}} {
    ...
    if {$fromid == $toid} {
        if {[info exists w]} then {
            ...
            vmdcon -error "clone_reps: cannot clone a representation on itself."
        }
        return
    }
    if { [lsearch -exact [molinfo list] $fromid] < 0} {
        if {[info exists w]} then {
            ...
            vmdcon -error "clone_reps: source molecule id $fromid does not exist"
        }
        return
    }
    if { !$togroup && [lsearch -exact [molinfo list] $toid] < 0} {
        if {[info exists w]} then {
            ...
            vmdcon -error "clone_reps: target molecule id $toid does not exist"
        }
        return
    }
    ...
    foreach toid $mlist {
        ...
        for {set i 0} {$i < [molinfo $fromid get numreps]} {incr i} {
            ...
            foreach {rep sel col mat} [molinfo $fromid get "{rep $i} {selection $i} {color $i} {material $i}"] {}
