#
# $Id: fftk_DihOpt.tcl,v 1.3 2012/01/26 22:10:17 johns Exp $
#
...
proc ::ForceFieldToolKit::DihOpt::sanityCheck { procType } {
    ...
    switch -exact $procType {
        opt {
            ...
            #  INPUT PAR DATA
            # ----------------
            # check parDataInput { {typedef} {k mult delta} ...}
...
proc ::ForceFieldToolKit::DihOpt::optimize {} {
    ...
    # in the form: {  {index def} {type def} { {k1 mult1 delta1} {k2 mult2 delta2} ...}  }
    ...
    # write out energies to log and energy comparison (EnComps) file, if selected
    #if { $WriteEnComps } {
    #    set EnComps [open "EnComps.dat" w]
    #    puts $EnComps "\# conformation\tQM\tMM(i)\tMM(f)"
    #}
    ...
    for {set i 0} {$i < [llength $EnQM]} {incr i} {
        ...
        #if { $WriteEnComps } { puts $EnComps "$i\t[lindex $EnQM $i]\t[lindex $EnMM $i]\t[lindex $EnMMf $i]" }
    }
    
    puts $outFile "END"; flush $outFile
    
    #if { $WriteEnComps } { close $EnComps }
    if { $debug } { puts $debugLog "\nQME, MMEi, MMEf written to log file and EnComps.dat"; flush $debugLog }
    ...
    foreach ele $finalParList {
        puts $outFile "[list dihedral [lrange $ele 0 3] [lindex $ele 4] [lindex $ele 5] [lindex $ele 6]]"
    }
    puts $outFile "END\n"
    flush $outFile
...
proc ::ForceFieldToolKit::DihOpt::refine {} {
    ...
    foreach ele $klist {
        ...
        } elseif { $ele < [expr -1*$refineKmax] } {
            lappend init [expr $refineKmax/3.0]
        } elseif { $ele > $refineKmax } {
            lappend init [expr $refineKmax/3.0]
        } else {
            lappend init $ele
        }
...
proc ::ForceFieldToolKit::DihOpt::parseGlog { inputFiles } {
    ...
    # localize ParaToolExt debugging variables
    variable debug
    variable debugLog
    variable guiMode
    ...
    foreach GlogFile $inputFiles {
        ...
        while {[eof $infile] != 1} {
            ...
            switch -regexp $inline {
                ...
                {SCF[ \t]*Done:} {
                    # parse calculated energy; convert hartrees to kcal/mol
...
proc ::ForceFieldToolKit::DihOpt::getMolecDihPar { psfFile pdbFile parinput } {
    ...
    # {
    #   {indices} {type def} { {k1 mult1 delta1} {k2 mult2 delta2} ...}
    # }
    
    # note: parinput is of form:
    # {
    #   {type def} {k mult delta}
    # }
    ...
    # build an array that matches typedef to parameter set (e.g., {k mult delta})
...
proc ::ForceFieldToolKit::DihOpt::calcMM { psfFile pdbFile toFit namdbin parlist } {
    ...
    set namdEn "namdenergy -silent -psf $psfFile -exe $namdbin -all -sel \$sel -cutoff 1000"
    foreach par $parlist {
    ...
    for {set i 0} {$i < [molinfo [$all molid] get numframes]} {incr i} {
        ...
        mol addfile $name.coor molid top
...
proc ::ForceFieldToolKit::DihOpt::pardata2klist { parData } {
    ...
    # {
    #   { {indices} {typedef1} { {k1 mult1 delta1} {k2 mult2 delta2} ...} }
    #   { {indices} {typedef2} { {k1 mult1 delta1}       ...            } }
    #   ...
    # }
    #         0         1             2,0                 2,1        2,X
    #
...
proc ::ForceFieldToolKit::DihOpt::klist2pardata { parData kList } {
    ...
    foreach ele $uniqueTypes {
        foreach paramSet [lindex $ele 2] {
            ...
            lappend paramSetNew [list $curK [lindex $paramSet 1] [lindex $paramSet 2]]
            incr count
        }
...
proc ::ForceFieldToolKit::DihOpt::prepDihPar { parData {kDihs {}} } {
    ...
    for {set j 0} {$j < [llength $uniqueTypes]} {incr j} {
        ...
        for {set n 0} {$n < [llength $curParList]} {incr n} {
            ...
            lappend parList [format "[lindex $uniqueTypes $j 1] %2.3f %s %3.2f" $curK $mult $delta]
        }
    }
...
proc ::ForceFieldToolKit::DihOpt::printSettings { printFile } {
    ...
    #puts $printFile "Write EnComps Files: $::ForceFieldToolKit::DihOpt::WriteEnComps"
