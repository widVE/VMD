...
package provide networkview 1.0
...
proc ::NetworkView::getEdgesByMetric { args } {
	...
	puts "    metric: edge associated value (edgeWeight, edgeBetweenness, edgeValue)"
...
proc ::NetworkView::drawNode { index radius resolution } {
    ...
    if {$objectIndex != -1} {
	#graphics $molid delete $nodes([lindex $objectIndices 0])
	graphics $molids($currentNetwork) delete $objectIndex
	#array unset nodes [lindex $objectIndices 0]
	lset nodes2 $currentNetwork $nodeObjectIndex $index -1
    }

    set active [lindex $nodes2 $currentNetwork $nodeActive $index]
    if {$active == 1} {
	graphics $molids($currentNetwork) color [lindex $nodes2 $currentNetwork $nodeColor $index]
...
proc ::NetworkView::getEdgeInfo { args } {
    ...
    } else if {$infoString == "betweenness"} {
	set infoType $edgeBetweenness
    } else if {$infoString == "color"} {
	set infoType $edgeColor
    } else if {$infoString != "value"} {
    ...
    foreach edgeIdPair $edgeIdPairs {
	#set edgeInfo $edges($currentNetwork,$edgeIdPair,$infoString)
	set ids [split $edgeIdPair ","]
	set edgeInfo [lindex $edges2 $currentNetwork $edgeColor [lindex $ids 0] [lindex $ids 1] $infoType]
	lappend info $edgeInfo
	#puts -nonewline "$edgeWeight "
    }
    #puts ""
