...
if {0} {  ;# Activate then 3 mode panes when feature is available
  ...
  grid [radiobutton $w.modetitle.create    -text "Create"  -indicatoron no -command {VolMapGUI::switch_mode_pane "generate"} -variable VolMapGUI::whichmode -value "generate"  -width 15]  -row 2 -column 1 -columnspan 1 -sticky ew -pady 2
...
#  grid columnconfigure $w.modetitle {1 2 3} -uniform 1
 grid columnconfigure $w.modetitle {1 2} -uniform 1
  ...
  $frame.voltypemenu.menu add radiobutton -label "coulomb (direct summation, slow)" -value coulomb -variable VolMapGUI::input_voltype -command {VolMapGUI::switch_voltype_pane "coulomb"}
  $frame.voltypemenu.menu add radiobutton -label "coulombmsm (multilevel summation, fast)" -value coulombmsm -variable VolMapGUI::input_voltype -command {VolMapGUI::switch_voltype_pane "coulombmsm"}
  ...
  # VOLTYPE: coulomb
  set frame $w.mode.gen.options.coulomb
  frame $frame
  ...
  # VOLTYPE: coulombmsm
  set frame $w.mode.gen.options.coulombmsm
  frame $frame
  grid [label $frame.reslabel -text "resolution:" -anchor w] -row 2 -column 1 -columnspan 1 -sticky ew -pady 2
  grid [entry $frame.res -textvariable ::VolMapGUI::input_res] -row 2 -column 3 -columnspan 1 -sticky ew -pady 2  
  grid [label $frame.resunits -text "A" -anchor w] -row 2 -column 4 -columnspan 1 -sticky ew -pady 2
  ...
  grid [radiobutton $w.output.usedestfile -text "Write to file:" \
    -variable ::VolMapGUI::output_destination -value "file"] -row 1 -column 1 -columnspan 1 -sticky w -pady 2
  grid [entry $w.output.destfile \
    -textvariable ::VolMapGUI::dest_filename] -row 1 -column 2 -columnspan 1 -sticky w -pady 2    
  grid [button $w.output.browse -text "Browse..." -command "VolMapGUI::dialog_getdestfile $w" -width 7] -row 1 -column 3 -columnspan 1 -sticky w -pady 2 -padx 2

  grid [radiobutton $w.output.usedestmol -text "Append to molecule:" \
    -variable ::VolMapGUI::output_destination -value "mol"] -row 2 -column 1 -columnspan 1 -sticky w -pady 2
  tk_optionMenu $w.output.destmol VolMapGUI::dest_molid_menu {}
  grid $w.output.destmol -row 2 -column 2 -columnspan 1 -sticky w -pady 2  
...
proc VolMapGUI::switch_voltype_pane {whichvoltype} {
  ...
  grid forget $frame.density $frame.distance $frame.occupancy $frame.mask $frame.ligand $frame.coulomb $frame.coulombmsm
...
proc VolMapGUI::update_mol_menus {args} {
  ...
  $menu delete 0 last
  $menu add radiobutton -label "same as input" -value same -variable VolMapGUI::dest_molid_menu
  $menu add separator
  $menu add radiobutton -label top -value top -variable VolMapGUI::dest_molid_menu
  $menu add radiobutton -label new -value new -variable VolMapGUI::dest_molid_menu
  if [llength $mollist] {
    $menu add separator
  } else {
    $menu entryconfigure 0 -state disabled
  ...
  foreach mol $mollist {
    $menu add radiobutton -label "$mol: [molinfo $mol get name]" -value $mol -variable VolMapGUI::dest_molid_menu
  }
}
...
proc VolMapGUI::go {} {
  ...
  if { $VolMapGUI::output_destination == "mol" }  {
    if { "$VolMapGUI::dest_molid_menu" == "new" } {
      set VolMapGUI::dest_molid [mol new]
      VolMapGUI::update_mol_menus
    } elseif { "$VolMapGUI::dest_molid_menu" == "same" } {
      set VolMapGUI::dest_molid $VolMapGUI::input_mol
    } else {
      set VolMapGUI::dest_molid $VolMapGUI::dest_molid_menu
    }
