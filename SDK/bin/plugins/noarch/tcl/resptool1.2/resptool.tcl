...
#             
# $Id: resptool.tcl,v 1.21 2011/03/09 22:42:32 johns Exp $
#
...
namespace eval ::RESPTool:: {
  ...
  proc initialize {} {
      ...
      variable respchargelist {}; # Our results
      variable callbackfctn {};   # User function to call when done

      variable respqwt   0.0005;  # def for stage 1
...
proc ::RESPTool::resp { sel args } {
   package require utilities

   variable gaussianinput [::util::getargs $args "-gau" {}]
   variable alignmol [::util::getargs $args "-alignmol" [$sel molid]]
   variable stage [::util::getargs $args "-stage" 1]

   set definput [file rootname [molinfo [$sel molid] get name]]_resp_stage${stage}.inp
   variable respinput [::util::getargs $args "-inp" $definput]

   ::RESPTool::resp_gui $sel

   if {[llength $gaussianinput]} {
      parse_gaussian_log
   }
   ...
   variable callbackfctn ::RESPTool::apply_charges
}

proc ::RESPTool::apply_charges { respchargelist } {
   variable origsel
   if {[$origsel num]!=[llength $respchargelist]} {
      puts "Wrong number of RESP charges"
      return
   }

   foreach charge $respchargelist {
      lappend chargelist [lindex $charge 1]
   }
   $origsel set charge $chargelist
}
...
proc ::RESPTool::resp_gui { sel } {
  ...
  button $w.out.binbutton -text "Find" -command {
     set ::RESPTool::respbinary [::ExecTool::find -interactive \
...
proc ::RESPTool::parse_gaussian_log {} {
   ...
   variable origsel
   set i 1
   foreach center $atomiccenters radius $atomicradii name [$origsel get name] {
      write_pdb_atom $fid $i $name {} ATM {} 1 {} $center 1 $radius RESP 
      incr i
   }
...
proc ::RESPTool::write_esp_file { file } {
   ...
   foreach atom $atomiccenters {
      ...
      puts $fid [format "%17s%16.7e%16.7e%16.7e" {} $x $y $z]
   }
...
proc ::RESPTool::set_view { } {
  ...
  # Load a copy of the original file
  set tmpfile "$env(TMPDIR)/RESP_[file rootname [molinfo $origmolid get name]].xbgf"
  $origsel writexbgf $tmpfile
...
proc ::RESPTool::start_picking { args } {
  ...
  mouse mode pick
  mouse callback on
  trace add variable ::vmd_pick_event write ::RESPTool::atom_picked_fctn
...
proc ::RESPTool::stop_picking { args } {
  ...
  mouse mode 0
  trace remove variable vmd_pick_event write ::RESPTool::atom_picked_fctn
...
proc ::RESPTool::find_equivalent_atom { molid index dist } {
   set sel [atomselect $molid "index $index"]
   set sim [atomselect $molid "atomicnumber [join [$sel get atomicnumber]] and not index $index"]
   set smile [::Paratool::get_all_smiles $molid $index $dist]
   set equilist {}
   foreach atom [$sim list] {
      set smile2 [::Paratool::get_all_smiles $molid $atom $dist]
      if {$smile==$smile2} {
	 lappend equilist $atom
      }
   }
   return $equilist
}
...
proc ::RESPTool::setup_stage_one {} {
   ...
   foreach atom [$all get {type resid segid atomicnumber index}] {
	 ...
	 set equiv [find_equivalent_atom $tmpmolid $index 3]
	 if {[llength $equiv]} {
