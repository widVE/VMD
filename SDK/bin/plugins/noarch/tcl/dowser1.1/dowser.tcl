...
proc ::Dowser::combine_waters { args } {
  ...
  foreach molid $dowserMolids {
    ...
    # Clamp the beta values in case the energies calculated by Dowser
    # don't fit in a valid PDB file
    set betaList [$sel get beta]
    for {set i 0} {$i < [llength $betaList]} {incr i} {
      if { [lindex $betaList $i] < -99 } {
        set betaList [lreplace $betaList $i $i -99]
      }
    }
    $sel set beta $betaList
    unset betaList
