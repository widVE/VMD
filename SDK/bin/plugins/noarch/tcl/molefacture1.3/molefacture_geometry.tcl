##
## $Id: molefacture_geometry.tcl,v 1.23 2011/04/04 04:06:42 johns Exp $
##
...
# Make atom selections for each neighbor, and their selection trees
  ...
  #foreach treesel $neighbortreesels {
  #  puts [$treesel get name]
  #}
...
# Find the two smallest fragments
  ...
  #puts "Switching selections $min1 and $min2"
...
###########################################################
 ...
 proc ::Molefacture::adjust_bondlength {} {
    ...
    set indexes1 [join [::util::bondedsel $tmpmolid $atom(2) $atom(1) -maxdepth [llength $atomlist]]]
    set indexes2 [join [::util::bondedsel $tmpmolid $atom(1) $atom(2) -maxdepth [llength $atomlist]]]
    #puts $indexes1
    #puts $indexes2
...
# Return if we don't have enough atoms to bother moving the dihedral
    ...
    variable tmpmolid
    set dihedral [measure dihed [list [list $dihedatom(0) $tmpmolid] [list $dihedatom(1) $tmpmolid] [list $dihedatom(2) $tmpmolid] [list $dihedatom(3) $tmpmolid] ] molid $tmpmolid]
    ...
    lappend dihedmarktags [graphics $tmpmolid sphere $bondcoor(0) radius 0.3 resolution 16]
    lappend dihedmarktags [graphics $tmpmolid sphere $bondcoor(3) radius 0.3 resolution 16]
...
#    set anglemoveatom "Atom1"
    ...
    variable tmpmolid
    set angle [measure angle [list [list $angleatom(1) $tmpmolid] [list $angleatom(2) $tmpmolid] [list $angleatom(3) $tmpmolid] ] molid $tmpmolid]
    ...
    lappend atommarktags [graphics $tmpmolid sphere $anglecoor(1) radius [expr $labelradius*1.5] resolution 15]
    lappend atommarktags [graphics $tmpmolid sphere $anglecoor(2) radius [expr $labelradius*1.5] resolution 15]
    lappend atommarktags [graphics $tmpmolid sphere $anglecoor(3) radius [expr $labelradius*1.5] resolution 15]
}
