##
## $Id: molefacture_state.tcl,v 1.21 2011/06/17 04:25:56 johanstr Exp $
##
...
proc ::Molefacture::update_openvalence { } {
   ...
   set n 0
   set myvallist [list]
   foreach vmdind [$sel list] bondorders [$sel getbondorders] elemind [$sel get atomicnumber] truecharge [$sel get charge] {
      if {$vmdind == ""} { continue }
      set nbonds 0.0
      ...
      lappend myvallist $myval
      lappend openvalencelist $openval
      lappend chargelist $charge
      set indarray($vmdind) $n
      incr n
   }
   
   set ind 0
   foreach sbonds [$sel getbonds] elemind [$sel get atomicnumber] charge $chargelist openval $openvalencelist {
      set element [lindex $periodic $elemind]
      ## To fix display of X--(PO4-)--X
      if {$element == "P" && $openval > 0} {
        set incrbonds 0
        set nearbyopenind [list]
        foreach b $sbonds {
            set bondedatomvalence [lindex $openvalencelist $indarray($b)]
            if { $bondedatomvalence > 0 } {
                lappend nearbyopenind $indarray($b)
            }
        }
        foreach n $nearbyopenind {
            if { [lindex $openvalencelist $n] > 0 && $openval > 0 } {
                incr incrbonds
                lset openvalencelist $n [expr [lindex $openvalencelist $n]-1]
                lset chargelist $n [expr [lindex $chargelist $n]+1]
                incr openval -1
                incr charge
            }
        }
        lset chargelist $ind $charge
        lset openvalencelist $ind $openval
      }
      incr ind
   }
   foreach vmdind [$sel list] name [$sel get name] sbonds [$sel getbonds] elemind [$sel get atomicnumber] truecharge [$sel get charge] type [$sel get type] myval $myvallist charge $chargelist openval $openvalencelist {
      if {$type == ""} {
        set type "X"
      }
      set element [lindex $periodic $elemind]
