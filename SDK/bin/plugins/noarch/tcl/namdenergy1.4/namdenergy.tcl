...
##      exe -- set path to namd executable explicitly
##
...
proc ::namdEnergy::namdenergy_usage {} {
  ...
  puts "namdEnergy)   -exe <path> (Set path to namd executable explicitly)"
...
proc ::namdEnergy::namdenergy { args } {
  ...
  foreach {i j} $arglist {
    ...
    if {$i=="-exe"}  then { set namdcmd "$j"; continue  }
...
proc ::namdEnergy::namdenergy-parse {file energytypes} {
  variable updateframes
  variable currentMol

  if {$updateframes != 0} {
    set outputfile [open ${jobname}-temp.log w]
    for {set i 0} {$i < [molinfo $currentMol get numframes]} {incr i [expr $skip + 1]} {
      set inputfile [open ${jobname}-temp.${i}.log r]
...
proc ::namdEnergy::namdmain { } {
  ...
  if {$use_biocore == 1} {
    if {!$silent} {puts "namdEnergy) Using BioCoRE"}
    set runext 1
...
proc ::namdEnergy::namdmain2 {} {
  ...
  # Concatenate the output from many runs, if needed
  if {$updateframes != 0} {
    set outputfile [open ${jobname}-temp.log w]
    for {set i 0} {$i < [molinfo $currentMol get numframes]} {incr i [expr $skip + 1]} {
      set inputfile [open ${jobname}-temp.${i}.log r]
...
proc ::namdEnergy::namdconf {} {
  ...
  if {$updateframes == 0} {
    ...
    foreach val [$sel1 get beta] {
      if {$val == 2} {
       error "Atom sets sel1 and sel2 are not disjoint, but must be in order to calculate interaction energies"
      }
    }
    ...
    set allmolsel [atomselect $currentMol all]

    for {set i 0} {$i < [molinfo $currentMol get numframes]} {incr i [expr $skip + 1]} {
      $allmolsel set beta 0
      $sel1 frame $i
      $sel1 update
      $sel1 set beta 1
      if {$nsel != 1} {
      ...
      $allmolsel frame $i
      $allmolsel writepdb ${jobname}-temp.${i}.pdb
...
# Set up topology and parameters
  ...
  if {$stype == "parm" || $stype == "parm7"} {
    puts $namdconf "1-4scaling 0.833333"
    puts $namdconf "scnb 2"
  }
...
# See if we want switching
  if {$switch == "" || $switch < 0.0} {
  ...
  if {$updateframes == 0} {
    ...
    for {set i 0} {$i < [molinfo $currentMol get numframes]} {incr i [expr $skip + 1]} {
      animate write dcd ${jobname}-temp.${i}.dcd waitfor all beg $i end $i $currentMol
    }
  }
...
proc ::namdEnergy::namdrun { } {
  ...
  # Run once if we're not updating selections, or many times if we are
  ...
  if {$updateframes == 0} {
    ...
    for {set i 0} {$i < [molinfo $currentMol get numframes]} {incr i [expr $skip + 1]} {
      # Put the appropriate pdb and dcd for this frame in the right place
      file rename -force ${jobname}-temp.${i}.pdb ${jobname}-temp.pdb
      file rename -force ${jobname}-temp.${i}.dcd ${jobname}-temp.dcd
      if {$runext == 0} {
        if {!$silent} {
          puts "namdEnergy) Running:"
          puts "namdEnergy) $namdcmd ${jobname}-temp.namd \n"
        ...
        if {$runextcommand == "biocore" } {
          ...
          set namdcommand $runextcommand
          set namdcommand [string map {INPUT ${jobname}-temp.namd LOGFILE ${jobname}-temp.${i}.log} $namdcommand]
          if {!$silent} {
            puts "namdEnergy) Running external analysis command $namdcommand"
            puts "namdEnergy) Input files: $remjob_inputfiles"
            puts "namdEnergy) Output files: $remjob_outputfiles"
...
#  puts "err1"
  ...
  grid [checkbutton $w.updatesels -text "Update selections every frame" \
  -variable [namespace current]::updateframes] \
