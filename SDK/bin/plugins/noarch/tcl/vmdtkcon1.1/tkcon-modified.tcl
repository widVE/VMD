...
# swap aliases around when vmd console is changed.
# XXX FIXME: we currently assume that widget mode logs to tkcon
#     technically any tk text widget could be used for logging.
#     before putting some code in place to resolve this, we need
#     to figure out, what the desired behavior should be.
#     at least with 'vmdcon -textmode' we can now tell tkcon
#     to give puts/gets back to the text console.
proc ::tkcon::reset_puts_gets {args} {
    variable PRIV

    switch [vmdcon -status] {
        text {
            set cmd [interp alias {} ::puts]
            if {($cmd != {}) && [string equal $cmd ::tkcon_puts] } then {
                interp alias {} ::puts {} ::tkcon_tcl_puts
            }
            set cmd [interp alias {} ::gets]
            if {($cmd != {}) && [string equal $cmd ::tkcon_gets] } then {
                interp alias {} ::gets {} ::tkcon_tcl_gets
            }
        ...
        widget {
            set cmd [interp alias {} ::puts]
            if {($cmd != {}) && [string equal $cmd ::tkcon_tcl_puts] } then {
                interp alias {} ::puts {} ::tkcon_puts
            }
            set cmd [interp alias {} ::gets]
            if {($cmd != {}) && [string equal $cmd ::tkcon_tcl_gets] } then {
...
proc ::tkcon::Init {} {
	...
	errorInfo	{}
	showOnStartup	1
	slavealias	{ edit more less tkcon vi vim pico gopython }
	slaveprocs	{
	    alias clear dir dump echo idebug lremove
	    tkcon_puts tkcon_gets observe observe_var 
            unalias which what tkcon_pwd tkcon_gopython
	}
	version		2.3
	RCS		{RCS: @(#) $Id: tkcon-modified.tcl,v 1.9 2011/03/16 15:31:00 johns Exp $}
	...
	set OPT(prompt1) {([file tail [::tkcon_pwd]]) [history nextid] % }
    }
    ...
    for {set i 0} {$i < $argc} {incr i} {
        ## XXX this break statement prevents TkCon from sourcing argv files
        break
    ...
    trace variable vmd_console_status w ::tkcon::reset_puts_gets
...
# declare this text field to be the general VMD console (if that extension is available).
    if {[info command vmdcon] != ""} then { 
        vmdcon -register $con output 
        vmdcon -widgetmode 
    }
...
proc ::tkcon::InitMenus {w title} {
	...
	$m add separator
	$m add command -label "Close" -underline 0 -accel Ctrl-q -command ::tkcon::Destroy
...
proc ::tkcon::MainInit {} {
	...
	if {[string match {} $slave]} {
      ...
      ## VMD Change: Do not quit VMD here, just hide TkCon!
	    tkcon hide
	} else {
...
##
## tkcon cannot handle interactive python as is. would be tricky to do
##
proc tkcon_gopython args {
    vmdcon -info "Running the python interpreter is not supported in tkcon."
    vmdcon -info "You have to use the real text mode console."
    return
}
...
# handle some common text mode full-screen commands 
# that can be problematic with tkcon.
interp alias {} ::more {} ::edit
interp alias {} ::less {} ::edit
interp alias {} ::vi   {} ::edit
interp alias {} ::vim  {} ::edit
interp alias {} ::pico {} ::edit
interp alias {} ::gopython {} ::tkcon_gopython
