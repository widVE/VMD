...
proc ::Membrane::membrane_usage { } {
    puts "Usage: membrane -l <lipid> -x <xsize> -y <ysize> {-o <prefix>} {-top <topology>}"
...
proc ::Membrane::membrane_core { args } {
    ...
    if ([info exists env(MEMBRANEDIR)]) {
      if { $topType == 36 } {
         set psffile $env(MEMBRANEDIR)/${lipid}36_box.psf
         set pdbfile $env(MEMBRANEDIR)/${lipid}36_box.pdb
      } else {
...
#      set topfile $env(MEMBRANEDIR)/top_all27_prot_lipid.inp
      ...
      if { $topType == 36 } {
         set psffile [file normalize [file dirname [info script]]]/${lipid}36_box.psf
         set pdbfile [file normalize [file dirname [info script]]]/${lipid}36_box.pdb
      } else {
