...
int VMDApp::VMDinit(int argc, char **argv, const char *displaytype, 
  ...
  if (!display) {
	  ...
	  {
		  display = new FionaVMDDisplayDevice();
	  }
#else
...
#ifdef FIONAVMD
  if (fionaNetSlave || fionaConf.appType == FionaConfig::DEVLAB)//FionaConfig::WINDOWED)
  {
...
int VMDApp::tool_delete(int toolnum) {
	if (!uivr) return FALSE;
#ifdef FIONAVMD
	Tool* t = uivr->gettool(toolnum);
	if (t)
	{
		const char *tracker = t->get_tracker();
		if (tracker != 0)
		{
			commandQueue->runcommand(new CmdToolDeleteDevice(tracker, 0));
		}

		const char *buttons = t->get_buttons();
		if (buttons != 0)
		{
			commandQueue->runcommand(new CmdToolDeleteDevice(buttons, 0));
		}
	}
#endif
...
#ifdef FIONAVMD
int VMDApp::tool_choose_tracker(int toolnum, const char *tracker) {
	if (!uivr) return FALSE;
	if (uivr->set_tracker(toolnum, tracker)) {
		commandQueue->runcommand(new CmdToolAddDevice(tracker, toolnum));
	}
	return FALSE;
}

int VMDApp::tool_choose_buttons(int toolnum, const char *buttons) {
	if (!uivr) return FALSE;
	if (uivr->set_buttons(toolnum, buttons)) {
		commandQueue->runcommand(new CmdToolAddDevice(buttons, toolnum));
	}
	return FALSE;
}
#endif
