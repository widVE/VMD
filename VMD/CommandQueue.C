...
void CommandQueue::execute_command(const char *sCmd)
{
	//performing some major safe casting here
	if (uilist.num() > 1)
	{
		if (UIObject *ui = uilist[1])
		{
			UIText *pText = dynamic_cast<UIText*>(ui);
			if (pText != 0)
			{
				TclTextInterp *pInterp = pText->get_tcl_text_interp();
				if (pInterp != 0)
				{
					pInterp->evalString(sCmd);
				}
			}
		}
	}
